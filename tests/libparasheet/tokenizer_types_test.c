...
int main() {
    ...
    PushToken(list, TOKEN_ID, example1);
    PushTokenID(list, TOKEN_LITERAL_STRING, example2, 42);
    ...
    for (u32 i = 0; i < list->size; i++) {
        ...
        printf("Token %u: type=%u, str='%.*s', symbolIndex=%u\n",
               i, t->type, t->string.size, t->string.data, t->symbolTableIndex);
    }
    ...
    if (popped) {
        printf("Popped token: type=%u, str='%.*s', symbolIndex=%u\n",
               popped->type, popped->string.size, popped->string.data, popped->symbolTableIndex);
    }
